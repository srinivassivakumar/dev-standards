...
7. Before committing or pushing the project branch, run `git status --short`.
8. Remove or exclude any dev-standards traces from the project repo.
9. Commit and push only the project changes required for the branch.

Do not require the user to repeat these cleanup and standards-update steps in the prompt.
